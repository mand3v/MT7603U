...
static INT32 MT_ATESetTxAntenna(RTMP_ADAPTER *pAd, CHAR Ant)
{
		...
		(MTK_REV_GTE(pAd, MT7628, MT7603E2)))
	{
		...
		if(pAd->E2pAccessMode == E2P_NONE){
			DBGPRINT(RT_DEBUG_INFO, ("%s, E2P_NONE\n",__FUNCTION__));
			rtmp_ee_efuse_read16(pAd, 0x35, &ePA); 
			if(ePA == 0){
				ePA = pAd->EEPROMImage[0x35];
				DBGPRINT(RT_DEBUG_INFO, ("EFUSE Value == 0, get value in Buffer:0x%x\n",__FUNCTION__));
			}
		}else if(pAd->E2pAccessMode == E2P_EFUSE_MODE){
			DBGPRINT(RT_DEBUG_INFO, ("%s, E2P_EFUSE_MODE\n",__FUNCTION__));
			rtmp_ee_efuse_read16(pAd, 0x35, &ePA); 
		}else
#endif /* RTMP_EFUSE_SUPPORT */
		{
			DBGPRINT(RT_DEBUG_INFO, ("%s, BIN/FLASH/EEPROM_MODE(0x%x), get value in buffer\n",__FUNCTION__, pAd->E2pAccessMode));
			ePA = pAd->EEPROMImage[0x35];
		}
			DBGPRINT(RT_DEBUG_INFO, ("%s, ePA:0x%x, E2pAccessMode:0x%x\n",__FUNCTION__,ePA,pAd->E2pAccessMode));
		if (ATECtrl->TxAntennaSel == 0)
		{
			RTMP_IO_READ32(pAd, CR_RFINTF_00, &Value);
			Value &= ~CR_RFINTF_CAL_NSS_MASK;
