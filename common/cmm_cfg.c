       ...
       /* Set some preconfigured value before interface up*/
		...
		case CMD_RTPRIV_IOCTL_USB_SUSPEND:
			...
			if ((pAd->WOW_Cfg.bEnable == TRUE) && INFRA_ON(pAd)){
				...
				ASIC_WOW_ENABLE(pAd);
				// TODO: Pat: Add PM4 when FW is ready
...
#endif
			{
				...
				CmdExtPmStateCtrl(pAd, BSSID_WCID, PM7, ENTER_PM_STATE);
		...
		case CMD_RTPRIV_IOCTL_USB_RESUME:
...
#endif
			{
				...
				CmdExtPmStateCtrl(pAd, BSSID_WCID, PM7, EXIT_PM_STATE);
			...
			if (pAd->WOW_Cfg.bEnable == TRUE){
				ASIC_WOW_DISABLE(pAd);
				// TODO: Pat: Add PM4 when FW is ready
				//CmdExtPwrMgtBitWifi(pAd, BSSID_WCID, PWR_ACTIVE);	//It is used for STA in Infra mode
				CmdExtPmStateCtrl(pAd, BSSID_WCID, PM4, ENTER_PM_STATE);	
